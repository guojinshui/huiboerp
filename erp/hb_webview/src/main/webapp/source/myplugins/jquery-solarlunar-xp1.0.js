(function ($) {
    $.fn.solarlunar = function (opts) {
        ...
        var minyear = option.minyear < 1892 ? 1892 : option.minyear;   // 因为农历对应公历的穷极法
																		// 对应范围只为1891-2100，又因为公历农历转化会用到上一年的。所以从1892开始。
        var maxyear = option.maxyear > 2100 ? 2100 : option.maxyear;
  
        // ****1.加载html****
        var html = "<div id='calendar' style='display:none;position:absolute;z-index:100;background-color:white;border-radius:8px;box-shadow:0px 0px 8px gray;padding:5px;'>";
        	html += "<input type='radio' name='calendar' value='1' id='gongli'>";
        	html += "<span style='cursor:pointer;'onclick='$(\"#gongli\").trigger(\"click\").trigger(\"click\");'>公历</span>";
        	html += "&nbsp;&nbsp;";
        	html += "<input type='radio' name='calendar' value='2' id='nongli'>";
        	html += "<span style='cursor:pointer;'onclick='$(\"#nongli\").trigger(\"click\").trigger(\"click\");'>农历</span>";
        	html += "<div id='slct'>";
        	html += "<span id='solar'>";
        	html += "<select name='year' id='year' class='select-down'></select>";
        	html += "<select name='month' id='month' class='select-down'>";
        	...
        	html += "</select>";
        	html += "<select name='day' id='day' class='select-down'>";
        	html += "</select>";
        	html += "</span>";
        	html += "<span id='lunar'>";
        	html += "<select name='lyear' id='lyear' class='select-down'>";
        	html += "</select>";
        	html += "<select name='lmonth' id='lmonth' class='select-down'>";
        	html += "</select>";
        	html += "<select name='lday' id='lday' class='select-down'></select>";
        	html += "</span>";
        	html += "</div>";
        	html += "</div>";
        $(this).after(html);
        ...
        $("#lyear").on("change", obj, function (event) {
            ...
            if ($lmonth_length == 13 || $lm.length == 13) {
                ...
                // $("#lmonth").find("option[text='二月']").attr("selected",true);
				// //将之前选中的月选中
            }
